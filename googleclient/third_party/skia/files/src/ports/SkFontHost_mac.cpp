    ...
    : SkScalerContext(desc), fLayout(0), fStyle(0)
{
    ...
    // Add a scale to the text to account for the difference in device DPI
    // between mac and windows.
    float device_scale = 96.0f / 72.0f;
    fTransform = CGAffineTransformMake(
        SkScalarToFloat(m[SkMatrix::kMScaleX]) * device_scale,
        SkScalarToFloat(-m[SkMatrix::kMSkewY]) * device_scale,
        SkScalarToFloat(-m[SkMatrix::kMSkewX]) * device_scale,
        SkScalarToFloat(m[SkMatrix::kMScaleY]) * device_scale,
