...
int CountFilesCreatedAfter(const FilePath& path,
  ...
  if (dir) {
    ...
    while (readdir_r(dir, &ent_buf, &ent) == 0 && ent) {
      ...
      struct STAT_TYPE st;
      int test = STAT_CALL(path.Append(ent->d_name).value().c_str(), &st);
      if (test != 0) {
...
bool Delete(const FilePath& path, bool recursive) {
  ...
  struct STAT_TYPE file_info;
  int test = STAT_CALL(path_str, &file_info);
  if (test != 0) {
...
bool PathExists(const FilePath& path) {
  struct STAT_TYPE file_info;
  return (STAT_CALL(path.value().c_str(), &file_info) == 0);
}

bool PathIsWritable(const FilePath& path) {
  FilePath test_path(path);
  struct STAT_TYPE file_info;
  if (STAT_CALL(test_path.value().c_str(), &file_info) != 0) {
...
bool DirectoryExists(const FilePath& path) {
  struct STAT_TYPE file_info;
  if (STAT_CALL(path.value().c_str(), &file_info) == 0)
...
bool GetFileInfo(const FilePath& file_path, FileInfo* results) {
  struct STAT_TYPE file_info;
  if (STAT_CALL(file_path.value().c_str(), &file_info) != 0)
    return false;
