...
//  Workaround because the string constant NSDefaultRunLoopMode moves between 
//  frameworks in different SDKs and causes cross-version compatibility 
//  problems.
//  This is the actual string value of NSDefaultRunLoopMode found by writing
//  a test app.
#define NS_DEFAULT_RUN_LOOP_MODE (@"kCFRunLoopDefaultMode")
...
void MessagePumpNSRunLoop::DoRun(Delegate* delegate) {
  while (keep_running_) {
    ...
    [[NSRunLoop currentRunLoop] 
        runMode:NS_DEFAULT_RUN_LOOP_MODE
    ...
    while (keep_running_) {
      ...
      NSEvent* event = [NSApp nextEventMatchingMask:NSAnyEventMask
                                          untilDate:[NSDate distantFuture]
                                             inMode:NS_DEFAULT_RUN_LOOP_MODE
                                            dequeue:YES];
      if (event) {
