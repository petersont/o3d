...
PluralFormat&
PluralFormat::operator=(const PluralFormat& rhs) {
    if (this != &rhs) {
        Format::operator=(rhs); 
        delete pluralRules;   
        delete fParsedValuesHash;
        ...
        UErrorCode status = U_ZERO_ERROR;
        locale = rhs.locale;
        pluralRules = rhs.pluralRules->clone();
        pattern = rhs.pattern;
        copyHashtable(rhs.fParsedValuesHash, status);
        numberFormat=NumberFormat::createInstance(locale, status);
        replacedNumberFormat=rhs.replacedNumberFormat;
    }
    return *this;
}
