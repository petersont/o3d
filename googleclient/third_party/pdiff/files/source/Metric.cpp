...
bool Yee_Compare(CompareArgs &args)
{
	...
	for (y = 0; y < h; y++) {
	  for (x = 0; x < w; x++) {
		...
		bool pass = true;
		float alphaDelta = fabsf(args.ImgA->Get_Alpha(index) - args.ImgB->Get_Alpha(index)) / 255.0f;
		...
		if (delta > factor * tvi(adapt)) {
			pass = false;
		} else if (alphaDelta > args.AlphaThreshold) {
			pass = false;
	...
	char different[100];
	sprintf(different, "%d pixels are different with a max alpha delta of %f\n", pixels_failed, maxAlphaDelta);
